    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <div style={{ marginBottom: isMobile ? '1rem' : '2rem', height: isMobile ? '120px' : '200px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {settings.logoUrl && settings.logoUrl.startsWith('http') ? (
               <img 
                 src={settings.logoUrl} 
                 alt="Logo" 
                 style={{ maxWidth: isMobile ? '120px' : '200px', maxHeight: isMobile ? '120px' : '200px', objectFit: 'contain', filter: 'drop-shadow(0 10px 20px rgba(0,0,0,0.2))', backgroundColor: 'rgba(255,255,255,0.1)', padding: '10px', borderRadius: '12px' }} 
               />
            ) : (() => {
               const IconComp = LucideIcons[settings.iconName] || LucideIcons.GraduationCap;
               return <IconComp size={isMobile ? 60 : 100} color="white" style={{ filter: 'drop-shadow(0 10px 20px rgba(0,0,0,0.2))' }} />;
            })()}
          </div>
          <h1 style={{ fontSize: isMobile ? '1.75rem' : '2.5rem', fontWeight: 800, marginBottom: '1rem', letterSpacing: '-0.025em' }}>{settings.schoolName || 'Institute Hub'}</h1>
        ...
        {/* Right Side: Form */}
        <div style={{ padding: isMobile ? '2rem 1.5rem' : '3.5rem', backgroundColor: 'white' }}>
          <div style={{ marginBottom: isMobile ? '1.5rem' : '2.5rem' }}>
            <h2 style={{ fontSize: isMobile ? '1.5rem' : '1.75rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Welcome Back</h2>
